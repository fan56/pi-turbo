 ...
 *
 * Usage:
 *   pi-opt [pi args...]    Launch pi with optimized extension loading
...
if (process.argv.includes("--status")) {
  try {
    ...
    console.log("\npi-opt per-extension timing (EMA ms):\n");
    console.log("  Extension".padEnd(50) + "EMA(ms)".padStart(8) + "  Runs".padStart(6));
    console.log("  " + "─".repeat(64));
    for (const [name, info] of entries) {
      const short = name.split("/").slice(-2).join("/");
      const runs = info.history ? info.history.length : 0;
      console.log("  " + short.padEnd(50) + String(Math.round(info.ema)).padStart(6) + "ms" + String(runs).padStart(6));
    }
    console.log("");
  } catch {
    console.log("No timing data yet. Run pi-opt once to collect data.");
  }
  process.exit(0);
}
