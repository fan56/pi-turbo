/**
 * A/B integration test: runs the full pi-turbo pipeline twice —
 * once serial (PI_TURBO_SERIAL=1) and once parallel — comparing timings.
...
function run(label, env) {
	...
	try {
		...
		// Extract pi-turbo timing lines
		for (const line of (stderr + stdout).split("\n")) {
			if (line.includes("[pi-turbo]")) console.log(line);
		}
		if (!stderr.includes("[pi-turbo]")) {
...
// A: Serial baseline
run("A: Serial (PI_TURBO_SERIAL=1)", { PI_TURBO_SERIAL: "1" });

// B: Parallel (default)
run("B: Parallel (default)", {});

console.log("\n✅ Done. Compare [pi-turbo] timing lines above.");
