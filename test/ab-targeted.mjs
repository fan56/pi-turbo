 ...
 *
 * Phase 1: Serial baseline (PI_TURBO_SERIAL=1)
...
const PI_TURBO_DIR = path.join(os.homedir(), ".pi-turbo");
const PER_EXT_FILE = path.join(PI_TURBO_DIR, "per-ext-timings.json");
...
function run(label, env = {}) {
	...
	const optLines = stderr
		.split("\n")
		.filter((l) => l.includes("[pi-turbo]"))
		.map((l) => l.trim());
...
try {
	fs.unlinkSync(PER_EXT_FILE);
} catch {}

console.log("=== Targeted Parallel A/B Test ===\n");

// Phase 1: Serial baseline
const serial = run("1. Serial baseline     ", { PI_TURBO_SERIAL: "1" });
