...
export async function initPiEnv() {
	...
	const httpUrl = pathToFileURL(
		path.join(piDistDir, "core/http-dispatcher.js"),
	).href;
...
export async function applyPatch() {
	try {
		...
		const { loadExtensionsCached, createExtensionRuntime } = await import(
			loaderUrl
		);
		...
		if (typeof original !== "function") {
			process.stderr.write(
				"[pi-turbo] loadFinalExtensionSet not found — skipping patch\n",
			);
			return;
		}
		...
		DefaultResourceLoader.prototype.loadFinalExtensionSet = async function (
			...
			// PI_TURBO_SERIAL=1 → bypass parallel loading (for A/B baseline)
			if (process.env.PI_TURBO_SERIAL === "1") {
			...
			try {
				...
				process.stderr.write(
					`[pi-turbo] targeted loader failed, falling back to serial: ${err.message}\n`,
				);
				return original.call(this, extensionPaths, preTrustExtensions);
			}
	...
	// Footer render caching (independent of extension loading patch)
	await applyFooterPatch(piDistDir);
}
