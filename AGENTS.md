...
pi-opt 是 `pi` 编码代理（`@earendil-works/pi-coding-agent`）的启动加速包装器，约 300 行代码。不是 fork——仅通过 monkey-patch `DefaultResourceLoader.prototype.loadFinalExtensionSet` 一个原型方法，将扩展加载从串行改为基于 EMA 时序画像的分块并行。

## 架构概览

```
bin/pi-opt.js          入口。用 --import src/patch.js 重新 exec node，然后调用上游 main()
...
src/config.js          常量：PI_OPT_DIR, EMA_ALPHA=0.3, MAX_HISTORY=100, 环境变量默认值
...
2. **patch 必须在 main() 之前生效** — `bin/pi-opt.js` 的 import 顺序是刻意设计，不可重排
...
| 变量 | 默认 | 作用 |
| ------ | ------ | ------ |
| `PI_OPT_SERIAL=1` | — | kill-switch，强制串行（等同未安装） |
| `PI_OPT_OFF=1` | — | 同上，完全跳过 patch |
| `PI_OPT_CHUNK` | 4 | 每个并行块包含的扩展数 |
| `PI_OPT_DEBUG=1` | — | 输出详细加载日志 |
| `PI_OPT_PROFILE=1` | — | 忽略现有画像，强制重新 profiling |

## 运行时数据

- `~/.pi-opt/state.json` — 持久化配置
- `~/.pi-opt/per-ext-timings.json` — EMA 时序画像（自学习，勿手动编辑）
